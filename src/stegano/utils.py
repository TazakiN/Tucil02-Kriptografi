"""
Utility functions untuk steganografi audio
Termasuk bitstream handling dan seed management
"""
...
def bytes_to_bits(data: bytes) -> List[int]:
    ...
    Args:
        data: Data dalam bentuk bytes
        
    Returns:
        List[int]: List bits (0 atau 1)
    """
    bits = []
    for byte in data:
...
def bits_to_bytes(bits: List[int]) -> bytes:
    """
    Konversi list of bits ke bytes
    
    Args:
        bits: List bits (0 atau 1)
        
    Returns:
        bytes: Data dalam bentuk bytes
    """
...
def get_lsb(sample: int, n: int = 1) -> int:
    """
    Ambil n LSB dari sample audio
    
    Args:
        sample: Sample audio (integer)
        n: Jumlah LSB yang diambil (1-4)
...
def set_lsb(sample: int, bits: int, n: int = 1) -> int:
    """
    Set n LSB dari sample audio
    
    Args:
        sample: Sample audio original
        bits: Nilai bits yang akan diset
        n: Jumlah LSB yang diset (1-4)
        
    Returns:
        int: Sample audio yang sudah dimodifikasi
    """
    mask = (1 << n) - 1  # Buat mask untuk n bits
...
def create_payload(secret_data: bytes, filename: str = "") -> bytes:
    """
    Buat payload yang berisi metadata + secret data
    Format: [filename_length(4)] [filename] [data_length(4)] [data]
    ...
    Returns:
        bytes: Payload lengkap dengan metadata
    """
...
def extract_payload(payload: bytes) -> Tuple[str, bytes]:
    """
    Extract metadata dan data dari payload
    
    Args:
        payload: Payload yang berisi metadata + data
        
    Returns:
        Tuple[str, bytes]: (filename, data)
        
    Raises:
        ValueError: Jika payload corrupt atau format salah
    """
    if len(payload) < 8:  # Minimal 4 bytes untuk filename_length + 4 bytes untuk data_length
    ...
    try:
        ...
        if offset + filename_length > len(payload):
            raise ValueError(f"Payload corrupt: filename length ({filename_length}) melebihi payload ({len(payload)})")
        ...
        if offset + data_length > len(payload):
            raise ValueError(f"Payload corrupt: data length ({data_length}) melebihi payload ({len(payload)})")
...
def generate_random_positions(total_samples: int, required_positions: int, seed: str = None) -> List[int]:
    """
    Generate posisi acak untuk menyisipkan data
    
    Args:
        total_samples: Total jumlah sample audio
        required_positions: Jumlah posisi yang dibutuhkan
        seed: Seed untuk random generator
        
    Returns:
        List[int]: List posisi acak yang sorted
        
    Raises:
        ValueError: Jika required_positions > total_samples
    """
    if required_positions > total_samples:
        raise ValueError(f"Required positions ({required_positions}) melebihi total samples ({total_samples})")
...
def calculate_capacity(total_samples: int, n_lsb: int) -> int:
    """
    Hitung kapasitas embedding dalam bytes
    
    Args:
        total_samples: Total jumlah sample audio
        n_lsb: Jumlah LSB per sample
        
    Returns:
        int: Kapasitas dalam bytes
    """
...
def int16_to_samples(data: bytes) -> List[int]:
    """
    Konversi raw audio data (16-bit) ke list of samples
    
    Args:
        data: Raw audio data
        
    Returns:
        List[int]: List of audio samples
    """
    samples = []
...
def samples_to_int16(samples: List[int]) -> bytes:
    """
    Konversi list of samples ke raw audio data (16-bit)
...
def test_utils():
    """
    Test function untuk utility functions
    """
    # Test bitstream conversion
    test_data = b"Hello!"
    bits = bytes_to_bits(test_data)
    recovered_data = bits_to_bytes(bits)
    print(f"Bitstream test: {test_data} -> {recovered_data} (Success: {test_data == recovered_data})")
    ...
    print(f"Payload test: '{filename}', {secret} -> '{recovered_filename}', {recovered_data}")
